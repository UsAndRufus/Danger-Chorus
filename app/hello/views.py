...
#this part is used to extract the common name of every animal(from db) and split it onto new lines 
...
<head>
    ...
    <script src="/static/audiofile.js"></script>
...
<h1 class="Courier New" {padding-right: auto; padding-left:auto;}>Danger Chorus</h1>
   ...
   <div id = 'listen'>
        <section id="play">
            ...
            <div class = 'button'>
                <button type="button" onclick="stopAnimals()">Stop all animals </button>
                </br>
            </div> 
        </section>
   </div>
  ...
  # The following code is used to put the 'common names'taken from the db into alphabetical order  
...
# the foloowing section is used to give the animal list a hyperlink each that will direct the user to the information exclusive to the selected animal 
#for each item of information about an animal its given a heading to explain each section 
